    ...
    <div className="grid gap-4 md:grid-cols-2">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Quick summary for triage. Each headline metric links to evidence and trust checks.
          </CardDescription>
        </CardHeader>
            ...
            <div key={m.key} className="rounded-md border p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {m.sources.slice(0, 2).map((src) => (
                  <ProvenanceBadge key={src.url} title={src.title} url={src.url} confidence={src.confidence} />
                ))}
              </div>
              <div className="mt-2">
